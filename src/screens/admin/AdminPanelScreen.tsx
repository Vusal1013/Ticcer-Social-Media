...
import { View, Text, FlatList, TouchableOpacity, Alert, StyleSheet, ActivityIndicator, Image, Modal, Pressable, TextInput } from 'react-native';
...
export default function AdminPanelScreen({ navigation }: any) {
  ...
  const [rejectTarget, setRejectTarget] = useState<{ reportId: string; reporterId: string } | null>(null);
  ...
  async function fetchGoldRequests() {
    const { data } = await supabase
      .from('gold_requests')
      .select('*, profile:profiles!user_id(full_name, username, avatar_url, verified_type)')
      .order('created_at', { ascending: false });
    if (data) setGoldRequests(data);
  }

  async function approveGold(requestId: string, userId: string) {
    Alert.alert('Gold təsdiqlə', 'Bu istifadəçinin Gold verified olmasını təsdiqləyirsən?', [
      { text: 'Ləğv et', style: 'cancel' },
      {
        text: 'Təsdiq et',
        onPress: async () => {
          const { error: updateError } = await supabase.from('profiles').update({ verified_type: 'gold' }).eq('id', userId);
          if (updateError) return Alert.alert('Xəta', updateError.message);
          await supabase.from('gold_requests').update({ status: 'approved' }).eq('id', requestId);
          await supabase.from('notifications').insert({
            user_id: userId,
            type: 'message',
            title: 'Gold verified!',
            body: 'Təbriklər! Gold verified oldunuz.',
            data: {},
          });
          fetchGoldRequests();
        },
  ...
  async function rejectGold(requestId: string, userId: string) {
    Alert.alert('Rədd et', 'Gold istəyi rədd edilsin?', [
      { text: 'Ləğv et', style: 'cancel' },
      {
        text: 'Rədd et',
        style: 'destructive',
        onPress: async () => {
          await supabase.from('gold_requests').update({ status: 'rejected' }).eq('id', requestId);
          await supabase.from('notifications').insert({
            user_id: userId,
            type: 'message',
            title: 'Gold istəyi rədd edildi',
            body: 'Gold verified istəyiniz rədd edildi. Ətraflı məlumat üçün adminlə əlaqə saxlayın.',
            data: {},
          });
          fetchGoldRequests();
        },
  ...
  async function handleApprove(reportId: string, reporterId: string, targetUserId?: string) {
    Alert.alert('Təsdiq et', 'Bu şikayəti təsdiq edib istifadəçiyə xəbərdarlıq edilsin?', [
      { text: 'Ləğv et', style: 'cancel' },
      {
        text: 'Təsdiq et',
        style: 'destructive',
        onPress: async () => {
          await supabase.from('reports').update({ status: 'approved', admin_note: 'Təsdiqləndi' }).eq('id', reportId);
          await supabase.from('notifications').insert({
            user_id: targetUserId || reporterId,
            type: 'message',
            title: 'Xəbərdarlıq',
            body: 'Sizə qarşı şikayət təsdiqləndi. Hesabınıza xəbərdarlıq edildi.',
            data: {},
          });
          fetchReports();
        },
  ...
  function openRejectModal(reportId: string, reporterId: string) {
    setRejectTarget({ reportId, reporterId });
    setRejectReason('');
  ...
  async function handleReject() {
    ...
    const { reportId, reporterId } = rejectTarget;
    const note = rejectReason.trim() || 'Səbəb göstərilmədi';
    await supabase.from('reports').update({ status: 'rejected', admin_note: note }).eq('id', reportId);
    await supabase.from('notifications').insert({
      user_id: reporterId,
      type: 'message',
      title: 'Şikayət cavablandırıldı',
      body: `Şikayətiniz dəyərləndirildi və rədd edildi. Səbəb: ${note}`,
      data: {},
    });
  ...
  async function handleDeleteReport(reportId: string) {
    Alert.alert('Reportu sil', 'Bu report tamamilə silinsin?', [
      { text: 'Ləğv et', style: 'cancel' },
      {
        text: 'Sil',
        style: 'destructive',
  ...
  async function handleDeleteGoldRequest(requestId: string) {
    Alert.alert('Gold istəyini sil', 'Bu gold istəyi tamamilə silinsin?', [
      { text: 'Ləğv et', style: 'cancel' },
      {
        text: 'Sil',
        style: 'destructive',
        onPress: async () => {
          const { error } = await supabase.from('gold_requests').delete().eq('id', requestId);
          if (error) return Alert.alert('Xəta', error.message);
          fetchGoldRequests();
        },
      },
  ...
  const renderReport = useCallback(({ item }: { item: any }) => (
    <View style={[styles.reportCard, { backgroundColor: colors.surface }]}>
      ...
      {item.status !== 'pending' && item.admin_note && (
        <View style={styles.reportBody}>
          <Text style={[styles.reportLabel, { color: colors.textMuted }]}>Admin qeydi</Text>
          <Text style={[styles.reportValue, { color: colors.text }]}>{item.admin_note}</Text>
        </View>
      )}
      ...
      {item.status === 'pending' ? (
        <View style={styles.reportActions}>
          <TouchableOpacity
            ...
            onPress={() => handleApprove(item.id, item.reporter_id, item.content?.user_id || item.content?.sender_id)}
          >
            <Text style={styles.reportActionText}>Təsdiq et (ceza)</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.reportActionBtn, { backgroundColor: colors.success }]}
            onPress={() => openRejectModal(item.id, item.reporter_id)}
          >
        ...
        <TouchableOpacity
          style={[styles.reportActionBtn, { backgroundColor: colors.textMuted + '40', marginTop: 10, alignSelf: 'stretch' }]}
          onPress={() => handleDeleteReport(item.id)}
        >
          <Ionicons name="trash-outline" size={16} color={colors.error} />
          <Text style={[styles.reportActionText, { color: colors.error, marginLeft: 6 }]}>Reportu sil</Text>
        </TouchableOpacity>
      )}
    </View>
  ), []);

  const renderGoldRequest = useCallback(({ item }: { item: any }) => (
    <View style={[styles.reportCard, { backgroundColor: colors.surface }]}>
      <View style={styles.reportHeader}>
        {item.profile?.avatar_url ? (
          <Image source={{ uri: item.profile.avatar_url }} style={{ width: 32, height: 32, borderRadius: 16 }} />
        ) : (
          <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ color: colors.white, fontWeight: fonts.weights.bold }}>{(item.full_name || '?')[0]}</Text>
          </View>
        )}
        <Text style={[styles.reportType, { color: colors.text }]}>{item.profile?.full_name || item.full_name}</Text>
        <View style={[styles.statusBadge, {
          backgroundColor: item.status === 'pending' ? colors.warning + '30' :
                          item.status === 'approved' ? colors.success + '30' : colors.error + '30'
        }]}>
          <Text style={[styles.statusText, {
            color: item.status === 'pending' ? colors.warning :
      ...
      <View style={styles.reportBody}>
        <Text style={[styles.reportLabel, { color: colors.textMuted }]}>Ad Soyad</Text>
        <Text style={[styles.reportValue, { color: colors.text }]}>{item.full_name}</Text>
      </View>

      <View style={styles.reportBody}>
        <Text style={[styles.reportLabel, { color: colors.textMuted }]}>Doğum tarixi</Text>
        <Text style={[styles.reportValue, { color: colors.text }]}>{item.dob}</Text>
      </View>

      {item.passport_image_url && (
        <Image source={{ uri: item.passport_image_url }} style={styles.reportImage} />
      )}
      ...
      {item.status === 'pending' ? (
        <View style={styles.reportActions}>
          <TouchableOpacity
            style={[styles.reportActionBtn, { backgroundColor: colors.success }]}
            onPress={() => approveGold(item.id, item.user_id)}
          >
            <Text style={styles.reportActionText}>Təsdiq et</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.reportActionBtn, { backgroundColor: colors.error }]}
            onPress={() => rejectGold(item.id, item.user_id)}
          >
            <Text style={styles.reportActionText}>Rədd et</Text>
          </TouchableOpacity>
        </View>
        ...
        <TouchableOpacity
          style={[styles.reportActionBtn, { backgroundColor: colors.textMuted + '40', marginTop: 10, alignSelf: 'stretch' }]}
          onPress={() => handleDeleteGoldRequest(item.id)}
        >
          <Ionicons name="trash-outline" size={16} color={colors.error} />
          <Text style={[styles.reportActionText, { color: colors.error, marginLeft: 6 }]}>Sil</Text>
        </TouchableOpacity>
      )}
    </View>
  ), []);
  ...
  return (
    <LinearGradient colors={['#0F0F23', '#1A1A3E']} style={styles.container}>
      ...
      <Modal visible={showRejectModal} transparent animationType="fade" onRequestClose={() => setShowRejectModal(false)}>
        <Pressable style={styles.rejectOverlay} onPress={() => setShowRejectModal(false)}>
          <Pressable style={[styles.rejectSheet, { backgroundColor: colors.surface }]} onPress={() => {}}>
            <Text style={[styles.rejectTitle, { color: colors.text }]}>Rədd etmə səbəbi</Text>
            <Text style={[styles.rejectDesc, { color: colors.textSecondary }]}>
              Bu səbəb şikayət edənə bildiriş olaraq gedəcək.
            </Text>
            <TextInput
            ...
            <View style={styles.rejectActions}>
              <TouchableOpacity
                style={[styles.rejectBtn, { backgroundColor: colors.textMuted + '30' }]}
                onPress={() => setShowRejectModal(false)}
              >
                <Text style={[styles.rejectBtnText, { color: colors.text }]}>Ləğv et</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.rejectBtn, { backgroundColor: colors.success }]}
...
const styles = StyleSheet.create({
  ...
  reportActionBtn: { flex: 1, borderRadius: 8, paddingVertical: 10, alignItems: 'center', flexDirection: 'row', justifyContent: 'center' },
  reportActionText: { color: colors.white, fontSize: fonts.sizes.sm, fontWeight: fonts.weights.bold },
  rejectOverlay: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)', padding: 32 },
  rejectSheet: { borderRadius: 16, padding: 24, width: '100%', maxWidth: 400 },
  rejectTitle: { fontSize: fonts.sizes.lg, fontWeight: fonts.weights.bold, marginBottom: 4 },
  rejectDesc: { fontSize: fonts.sizes.sm, marginBottom: 16, lineHeight: 18 },
  rejectInput: { borderRadius: 12, borderWidth: 1, padding: 14, fontSize: fonts.sizes.md, minHeight: 80 },
  rejectActions: { flexDirection: 'row', gap: 10, marginTop: 16 },
  rejectBtn: { flex: 1, borderRadius: 10, paddingVertical: 12, alignItems: 'center' },
  rejectBtnText: { color: colors.white, fontSize: fonts.sizes.md, fontWeight: fonts.weights.bold },
});
