...
export default function ConversationsListScreen({ navigation, route }: any) {
  ...
  const shareProfile = route?.params?.shareProfile;
  const shareItem = sharePost || shareReel || shareProfile;
  const shareType = sharePost ? 'post' : shareReel ? 'reel' : shareProfile ? 'profile' : null;
  const { user } = useAuth();
  ...
  async function handleConversationPress(item: Conversation) {
    if (shareItem) {
      let metadata: any = { type: shareType, id: '' };
      let content = '';
      if (shareType === 'post') {
        metadata = {
          type: 'post',
          ...
          likes_count: sharePost.likes_count || 0,
          comments_count: sharePost.comments_count || 0,
          profile_name: sharePost.profile?.full_name || '',
          profile_avatar: sharePost.profile?.avatar_url || null,
          profile_username: sharePost.profile?.username || '',
        };
        const preview = sharePost.content || '';
        content = preview.length > 80 ? preview.slice(0, 80) + '...' : preview;
      } else if (shareType === 'reel') {
        metadata = {
          type: 'reel',
          id: shareReel.id,
          description: shareReel.description || '',
          thumbnail_url: shareReel.thumbnail_url || null,
          likes_count: shareReel.likes_count || 0,
          profile_name: shareReel.profile?.full_name || '',
          profile_avatar: shareReel.profile?.avatar_url || null,
          profile_username: shareReel.profile?.username || '',
        };
        const preview = shareReel.description || '';
        content = preview.length > 80 ? preview.slice(0, 80) + '...' : preview;
      } else if (shareType === 'profile') {
        metadata = {
          type: 'profile',
          id: shareProfile.id,
          full_name: shareProfile.full_name || '',
          username: shareProfile.username || '',
          avatar_url: shareProfile.avatar_url || null,
          bio: shareProfile.bio || '',
          verified_type: shareProfile.verified_type || 'none',
        };
        content = `${shareProfile.full_name || 'Profil'} paylasildi`;
      }
