...
| 15 | **Chat silmə sistemi** — ConversationsListScreen-də long press → bottom sheet: "Mənim üçün sil" (yalnız öz participant qeydini silir) / "Hamı üçün sil" (bütün mesaj+participant+conversation silinir). RLS DELETE policy-lər əlavə edildi (`messages`, `conversation_participants`, `conversations`). Siyahı navbar altında qalmaması üçün `ListFooterComponent` əlavə edildi |
| 16 | **Toplu mesaj silmə** — ChatScreen-də "Seç" düyməsi ilə seçim rejimi, hər mesajda checkbox, long press ilə avtomatik seçim, "Seçilmişləri sil (N)" paneli, `Imtina` ilə çıxış |
