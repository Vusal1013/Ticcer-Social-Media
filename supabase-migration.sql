...
DROP POLICY IF EXISTS "Lives viewable by everyone" ON lives;
DROP POLICY IF EXISTS "Users can start live" ON lives;
DROP POLICY IF EXISTS "Users can update own live" ON lives;
...
DROP POLICY IF EXISTS "Users can view own saved reels" ON saved_reels;
...
DROP POLICY IF EXISTS "Admins can delete gold_requests" ON gold_requests;
...
-- Enable Realtime for notifications (in-app banner)
